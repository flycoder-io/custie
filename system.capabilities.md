...
- `custie slack history <name-or-id> [--today] [--limit n] [--oldest ts] [--latest ts]` — Read channel top-level messages (does NOT include thread replies)
- `custie slack thread <name-or-id> <ts>` — Read a thread: parent message + all replies. Use this when you need to see thread context.
